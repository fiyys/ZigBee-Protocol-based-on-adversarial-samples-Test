...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, labels) in enumerate(tqdm(train_dataloader, desc="Training")):
            ...
            noise_item = random.sample(noise_list, 1)[0]
            noise = torch.load('./noise/'+noise_item).to(device)
            optimizer.zero_grad()
            outputs = model(inputs+alpha*noise)
        ...
        if bset_accuracy<accuracy:
            ...
            torch.save(model.state_dict(), 'Best_model_ResNet18_adv_weights.pth')
        print(f"Accuracy: {accuracy}, Bset_accuracy: {bset_accuracy}")

    torch.save(model.state_dict(),'./model_ResNet18_adv_weights.pth')
