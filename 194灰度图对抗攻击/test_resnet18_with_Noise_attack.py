...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('./Best_model_ResNet18_adv_weights.pth'))
    # 损失函数和优化器
    ...
    for inputs, labels in tqdm(test_dataloader, desc="Evaluating"):
        ...
        pert = torch.FloatTensor(*inputs.shape).uniform_(-alpha, alpha).to(device)
        # pert带有梯度
        pert.requires_grad_()
        # 为输入数据增加对抗性扰动pert
        out = model(inputs + pert)
        # 因为loss的梯度一直是累加的，所以每个step贡献1/M的grad值
        loss = criterion(out, labels)/M
        ...
        outputs = model(inputs+pert.detach())
